...
.ntisas-confirm-image .prebooru-select-controls {
    left: 0;
    bottom: -2.5em;
}
...
function RenderPostPreview(post,server_url,append_html="",populate_title=true) {
    ...
    return `
<article class="prebooru-post-preview ntisas-post-selectable" data-id="${post.id}" data-size="${post.size}">
...
function RenderTwimgPreview(image_url,index,selectable) {
    ...
    return `
<article class="prebooru-post-preview ntisas-tweet-preview ${selected_class}" data-id="${index}">
    <div class="ntisas-image-container">
        ${image_html}
    </div>
    ${append_html}
...
function GetSelectPostIDs(tweet_id,type) {
    ...
    let $select_previews = $('.ntisas-post-select', PREBOORU[type][tweet_id][0]);
    return JSPLib.utility.getDOMAttributes($select_previews, 'id', Number);
}

function SelectPreview(event) {
    $(event.currentTarget).closest('.prebooru-post-preview').toggleClass('ntisas-post-select');
    event.preventDefault();
}
