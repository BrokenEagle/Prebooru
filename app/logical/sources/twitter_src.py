...
@check_guest_auth
def twitter_request(url, method='GET', wait=True, httpx=False):
    check_request_wait(wait)
    reauthenticated = False
    rq_methods = REQUEST_METHODS if not httpx else XREQUEST_METHODS
    for i in range(3):
        try:
...
def get_search_page(query, cursor=None):
    ...
    if cursor is not None:
        params['cursor'] = cursor
    url_params = urllib.parse.urlencode(params)
    return twitter_request("https://api.twitter.com/2/search/adaptive.json?" + url_params, httpx=True)
