...
# ## PACKAGE IMPORTS
from config import PIXIV_PHPSESSID, DEBUG_MODE
...
def partial_image_url(image_url, size=None):
    ...
    if match:
        ...
        if size == 'regular':
            return "/img-master/img/%s/%s_p%s_master1200.jpg" %\
                   (match.group('date'), match.group('id'), match.group('order'))
        if size == 'small':
            return "/c/540x540_70/img-master/img/%s/%s_p%s_master1200.jpg" %\
                   (match.group('date'), match.group('id'), match.group('order'))
        if size == 'thumb':
            return "/c/250x250_80_a2/img-master/img/%s/%s_p%s_square1200.jpg" %\
                   (match.group('date'), match.group('id'), match.group('order'))
        if size == 'mini':
            return "/c/128x128/img-master/img/%s/%s_p%s_square1200.jpg" %\
                   (match.group('date'), match.group('id'), match.group('order'))
    return None
