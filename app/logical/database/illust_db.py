...
from .base_db import set_column_attributes, set_relationship_collections, append_relationship_collections,\
    set_timesvalue, set_association_attributes, add_record, delete_record
...
ANY_WRITABLE_COLUMNS = ['site_id', 'site_illust_id', 'site_created', 'pages', 'score', 'active']
...
def create_illust_from_parameters(createparams, commit=True):
    return set_media_asset_from_parameters(Illust(), createparams, commit, 'created')
...
def update_illust_from_parameters(illust, updateparams, commit=True):
    return set_media_asset_from_parameters(illust, updateparams, commit, 'updated')
...
def set_illust_from_parameters(illust, setparams, commit, action):
    ...
    col_result = set_column_attributes(illust, ANY_WRITABLE_COLUMNS, NULL_WRITABLE_ATTRIBUTES, setparams):
    if col_result:
        commit_or_flush(False, safe=True)
    rel_result = _set_relations(illust, setparams)
    url_result = _set_illust_urls(illust, setparams['illust_urls'])
    if any([rel_result, url_result]):
        illust.updated = get_current_time()
        commit_or_flush(commit)
        print("[%s]: %s" % (illust.shortlink, action))
    return illust
