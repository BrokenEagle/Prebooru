...
class SequentialPaginate():
    def __init__(self, query=None, per_page=DEFAULT_PAGINATE_LIMIT, page=None, count=None, distinct=False,
        ...
        if self.current_count > 0:
            ...
            self.page_max = max([item.id for item in self.items])
            self.page_min = min([item.id for item in self.items])
            if self.direction == 'a':
                self.range = f'{self.page_min} - {self.page_max}'
            else:
                self.range = f'{self.page_max} - {self.page_min}'
        else:
    ...
    def above_pagenum(self):
        return f'a{self.page_max}' if self.current_count > 0 else None

    @property
    def below_pagenum(self):
        return f'b{self.page_min}' if self.current_count > 0 else None

    @property
    def has_above(self):
        return self.max_id > self.page_max if self.current_count > 0 else False

    @property
    def has_below(self):
        return self.min_id < self.page_min if self.current_count > 0 else False
