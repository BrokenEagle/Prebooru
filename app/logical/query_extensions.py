...
class SequentialPaginate():
    ...
    def above_pagenum(self):
        return f'a{self.page_max}'

    @property
    def below_pagenum(self):
        return f'b{self.page_min}'

    @property
    def has_above(self):
        return self.max_id > self.page_max

    @property
    def has_below(self):
        return self.min_id < self.page_min
    ...
    def _get_items(self):
        ...
        if self.distinct:
            ...
            q = self.query
        q = q.filter(model.id > self.sequential_id) if self.direction == 'a' else q.filter(model.id < self.sequential_id)
        return q.order_by(model.id.asc()).limit(self.per_page).all()
