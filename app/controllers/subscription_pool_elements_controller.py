...
from ..logical.records.post_rec import archive_post_for_deletion
from ..logical.database.subscription_pool_element_db import update_subscription_pool_element_expires,\
    update_subscription_pool_element_active, update_subscription_pool_element_deleted, get_elements_by_id,\
    update_subscription_pool_element_keep
...
def delete_element_post(element):
    if element.post is None:
        return "No posts to delete."
    else:
        result = archive_post_for_deletion(element.post)
        if result['error']:
            return result['message']
        update_subscription_pool_element_active(element, False)
...
@bp.route('/subscription_pool_elements/<int:id>/unexpire', methods=['POST'])
def unexpire_html(id):
    element = get_or_abort(SubscriptionPoolElement, id)
    update_subscription_pool_element_expires(element, None)
    return redirect(request.referrer)


@bp.route('/subscription_pool_elements/<int:id>/unexpire.json', methods=['POST'])
def unexpire_json(id):
    element = get_or_error(SubscriptionPoolElement, id)
...
@bp.route('/subscription_pool_elements/<int:id>/delete_post', methods=['DELETE'])
def delete_post_html(id):
    element = get_or_abort(SubscriptionPoolElement, id)
    error = delete_element_post(element)
    if error is not None:
        flash(error, 'error')
    return redirect(request.referrer)


@bp.route('/subscription_pool_elements/<int:id>/delete_post.json', methods=['DELETE'])
def delete_post_json(id):
    element = get_or_error(SubscriptionPoolElement, id)
